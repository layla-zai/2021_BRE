...
# value doesn't exist return NA
iris[c(37, 49, 132, 12, 77, 200), c(1, 3)]
```
...
iris[iris$Species != 'virginica',] %>% tail()
```
...
iris %>%
  select(Petal.Length:Species) -> select_example
select_example

iris %>%
  select(species = Species, sepal_length = Sepal.Length, sepal_width = Sepal.Width)
...
iris %>%
  mutate(flower_size = case_when(Sepal.Length < 3 ~ 'small',
                                 ...
                                 TRUE ~ 'large')) %>%
  distinct(Sepal.Length, flower_size) %>% arrange(Sepal.Length)
```
